    ...
    if mask_enable:
        ...
        if diffuse:
            ...
            for i in range(seg_ids.shape[0]):
                ...
                # mask_rf[region] = temp_mask[region].mean()*0.9+mask_rf[region]*0.1
            ...
            if vis_diffuse:
                ...
                ax[row_id2, 0].imshow(mark_boundaries(temp_im, superpix))
                ax[row_id2, 0].set_title("Superpixel Boundary")
                ax[row_id2, 1].imshow(mark_boundaries(temp_im, seg_bd))
                ax[row_id2, 1].set_title('Deep Net Prediction' )
                # ax[row_id2, 1].imshow(mark_boundaries(mask, superpix))
                # ax[row_id2, 1].set_title('soft deep mask' )
                ax[row_id2, 2].imshow(mark_boundaries((temp_mask*color_base).astype(np.uint8), superpix))
                ax[row_id2, 2].set_title('Deep Net Confidence Map')
                # ax[row_id2, 1].imshow(mark_boundaries(mask_rf, superpix))
                # ax[row_id2, 1].set_title('rf soft mask')
                ax[row_id2, 3].imshow(mark_boundaries(temp_im, seg_bd2_rf))
                ax[row_id2, 3].set_title('Semiparametric Prediction')
                ax[row_id2, 4].imshow(mark_boundaries( (mask_in_img*color_base).astype(np.uint8), superpix))
                ax[row_id2, 4].set_title('Semiparametric Confidence Map')
                # ax[0, 2].imshow(mark_boundaries(im, state['mask_nn']>p.seg_thr))
                # ax[0, 2].set_title('nn mask')
                ax[row_id2, 5].imshow(mark_boundaries(im, mask_in_img>p.seg_thr))
                ax[row_id2, 5].set_title('diffuse mask')
                # if img_neg !=0:
                ...
                if semip:
                    ...
                    def deeprf_train(llfeat,mask, rf_model,lr_model):
                        ...
                        if visualize:
                            ...
                            row_id2 = obj_id%num_fig
                            ax2[row_id2,0].imshow(im)
                            ax2[row_id2,0].set_title("Image")
                            ax2[row_id2,1].imshow((labp1.reshape(sz1,sz2)*100).astype(np.uint8))
                            ax2[row_id2,1].set_title('Random Forest Prediction')
                            # ax[1,0].imshow( (labp2.reshape(sz1,sz2)*100).astype(np.uint8))
                            # ax[1,0].set_title('Linear Model Prediction')
                            ax2[row_id2,2].imshow(labp)
                            ax2[row_id2,2].set_title('Semiparametric Prediction')
                            ax2[row_id2,3].imshow(mask)
                            ax2[row_id2,3].set_title('Mask Ground Truth')
                            plt.tight_layout()
                            # fig_counter2+=1
                            if obj_id %num_fig ==0:
                                # fig_counter2 =0
...
def main():
    ...
    for v_id, video in list(enumerate(dataset.keys(), start=1))[::-1]:
