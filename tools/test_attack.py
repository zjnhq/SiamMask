...
def superpixel_seg(img, mask = 0,visualize_ =0, n_segments=150):
    # set_trace()
    # exm_img= astronaut()
    ...
    superpix_slic = slic(img, n_segments=n_segments, compactness=20, sigma=1)
...
parser.add_argument('--dataset', dest='dataset', default='DAVIS2016', choices=dataset_zoo,
                    help='datasets')
...
def get_subwindow_tracking(im, pos, model_sz, original_sz, avg_chans, out_mode='torch', substitute_patch=0):
    ...
    # set_trace()
    # if not isinstance(substitute_patch,int):
    #     # set_trace()
    #     # patch = substitute_patch.data.cpu().numpy()
    #     patch = substitute_patch.data.squeeze(0).cpu().numpy().transpose(1,2,0)
    #     y_dist =int(context_ymax + 1) -int(context_ymin)
    #     x_dist = int(context_xmax + 1)- int(context_xmin)
    #     patch = cv2.resize(patch, (y_dist, x_dist))
    #     im[int(context_ymin):int(context_ymax + 1), int(context_xmin):int(context_xmax + 1), :] = patch
    #     return im
...
def siamese_init(im, target_pos, target_sz, model, hp=None, device='cpu', semip =False):
    ...
    if semip:
        state['semip'] = True
    else:state['semip'] = False

    state['semip_time'] = 0

    return state

def semip_predict(net, im, rf_model, lr_model, save_name="save_name", multiclass=1):
    o_id = 1
    ...
    labp1 = rf_model.predict_proba(pixels)
    if multiclass:
        nclasses= labp1.shape[1]
        labp2 =lr_model.predict_proba(pixels)
        labp = labp1*0.8+ labp2 *0.2
        labp=labp.reshape(sz1,sz2,nclasses)
        ...
        use_svm =0
        if use_svm:
            labp2 = lr_model.predict_proba(pixels)[:,o_id]
        ...
        labp1=labp1.reshape(sz1,sz2)
        labp2 =labp2.reshape(sz1,sz2)
...
def siamese_track(state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False, visualize_=0,save_name='exp', labp=0,mask_gt=0):
    p = state['p']
    net = state['net']
    ...
    x_crop.requires_grad=True
    # net.train()

    # for try_iters in range(2):
    ...
    visualize= 1
    # set_trace()
    # mask_gt_crop = get_subwindow_tracking(mask_gt, target_pos, p.instance_size, round(s_x), avg_chans).unsqueeze(0)

    
    # mask_fake_crop = 1 - mask_gt_crop
    # adv_loss = get_cls_loss(mask,mask_fake_crop)
    ...
    #     if labp==0: labp = semip_predict(net, im, rf_model, lr_model,multiclass=0)
    ...
    if mask_enable:
        ...
        if adv_attack:
            ...
            optimizer_ad = torch.optim.Adam([x_crop], lr=0.5)
            mask_pred = net.track_refine((delta_y, delta_x)).sigmoid()#.squeeze()
            mask_fake_crop = (1.0 - mask_pred).detach()
            patch_orig = x_crop.data.squeeze(0).cpu().numpy().transpose(1,2,0)
            set_trace()
            labp_orig = semip_predict(net, patch_orig, rf_model, lr_model)[:,:,1]

            # mask_fake_crop.detach()
            # adv_loss = select_cross_entropy_loss(mask_pred,mask_fake_crop)
            # dtype=torch.cuda.LongTensor
            # mask_fake_crop torch.log(mask_fake_crop)
            for iters in range(10):

                adv_loss = criterion(mask_pred, mask_fake_crop.detach())
                # set_trace()
                adv_loss.backward(retain_graph=True)
                optimizer_ad.step()
                print("adv_loss:"+str(adv_loss))
                print("x_crop grad:"+str(x_crop.grad.abs().max()))
                print("x mean:"+str(x_crop.mean()))
                print("mask_pred:"+str(mask_pred.mean()))

                if mask_enable:
                    score2, delta, mask = net.track_mask(x_crop.to(device))
                else:
                    score2, delta = net.track(x_crop.to(device))
                mask_pred = net.track_refine((delta_y, delta_x)).sigmoid()#.squeeze()
                # pscore2 = penalty * score2

                # # cos window (motion model)
                # pscore2 = pscore2 * (1 - p.window_influence) + window * p.window_influence
                # best_pscore_id = np.argmax(score2)
                # best_pscore_id_mask = np.unravel_index(best_pscore_id, (5, p.score_size, p.score_size))
                # delta_x, delta_y = best_pscore_id_mask[2], best_pscore_id_mask[1]
            # set_trace()
            patch_ad = x_crop.data.squeeze(0).cpu().numpy().transpose(1,2,0)
            labp_ad = semip_predict(net, patch_ad, rf_model, lr_model)[:,:,1]
            if visualize and semip:
                fig, ax = plt.subplots(2, 3, figsize=(20, 20))
                
                ax[0,0].imshow(patch_orig.astype(np.uint8))
                ax[0,0].set_title("patch_orig ")
                ax[0,1].imshow(labp_orig)
                ax[0,1].set_title('labp_orig')
                ax[1,0].imshow(patch_ad.astype(np.uint8))
                ax[1,0].set_title("patch_ad ")
                ax[1,1].imshow(labp_ad)
                ax[1,1].set_title('labp_ad')
                plt.show()
                # im = get_subwindow_tracking(im, target_pos, p.instance_size, round(s_x), avg_chans, substitute_patch= x_crop)
        ...
        if semip:
            dsize = (mask_in_img.shape[1],mask_in_img.shape[0])
            labp =cv2.resize(labp, dsize=dsize, interpolation=cv2.INTER_CUBIC)
            target_ind = np.where(mask_in_img > 0.05)
        ...
        if diffuse:
            ...
            seg_bd = np.logical_and(temp_mask>(p.seg_thr-0.2),temp_mask<(p.seg_thr+0.25))
            ...
            temp_labp = labp[roi_ymin:roi_ymax,roi_xmin:roi_xmax]

            use_superpixel = 0
            temp_labp_pooled = temp_labp.copy()
            if use_superpixel:
                n_segments = 200
                for hier_diff_id in range(1):
                    superpix = superpixel_seg(temp_im,visualize_=0,n_segments = n_segments)
                    ...
                    for i in range(seg_ids.shape[0]):
                        ...
                        # mask_rf[region] = mask_rf[region].mean()*0.4+mask_rf[region]*0.6
                        temp_labp_pooled[region] = temp_labp_pooled[region].mean()*0.8+temp_labp_pooled[region]*0.2
                    # mask_rf[region] = temp_mask[region].mean()
                    n_segments = n_segments+ 150
            use_igf = 1
            if use_igf:
                r = 16
                e = 0.0002
                GF = GuidedFilter(temp_im, radius=r, eps=e)
                # mask_rf = GF.filter(mask_rf)
                
                labp_gf=GF.filter(temp_labp)
                temp_labp_filted_pooled = GF.filter(temp_labp_pooled)
                mask_rf = temp_labp_filted_pooled
            ...
            vis_diffuse=1
            if state['frame_id']==1 and vis_diffuse:
                fig, ax = plt.subplots(1, 5, figsize=(20, 20))
                num_fig = 4
                # row_id2 = obj_id%num_fig
                ax[0].imshow(temp_labp)
                ax[0].set_title("SDForest Confidence")
                ax[1].imshow(temp_labp_pooled)
                ax[1].set_title("Confidence pooled")
                ax[2].imshow(labp_gf)
                ax[2].set_title("Confidence filtered")

                ax[3].imshow(mark_boundaries(temp_im, labp_gf>p.seg_thr,mode='thick'))
                ax[3].set_title('SDForest Prediction')
                # ax[4].imshow(mark_boundaries(temp_im, temp_mask>p.seg_thr,mode='thick'))
                ax[4].imshow(temp_mask>p.seg_thr)
                ax[4].set_title('SiamMask Prediction')
                # if img_neg !=0:
                ...
                # if state['frame_id']==1: 
                # plt.savefig(save_name+"_vis_igf.eps")
                # plt.savefig(save_name+"_vis_igf.jpg")

                print("save to "+save_name+"_vis_igf.eps" )
...
def semip_init(model,im,mask, name = 'default'):
    ...
    lr_model = LogisticRegression(C=50.0, random_state=0, solver='lbfgs', max_iter=15, n_jobs=-1)
    ...
    def deeprf_train(llfeat,mask, rf_model,lr_model):
        ...
        ave_indx = np.arange(0, lab.size)[::6]
        ...
        #     # labp = np.maximum(sv_model.decision_function(labp_concat),-0.1)+0.1
        #     labp = labp.reshape(sz1,sz2)
        labp2 = rf_model.predict_proba(pixels)

        labp = labp1*0.8+ labp2*0.2
        labp1,labp2,labp = labp1.argmax(1),labp2.argmax(1),labp.argmax(1)
        labp1,labp2,labp = labp1.reshape(sz1,sz2),labp2.reshape(sz1,sz2),labp.reshape(sz1,sz2)
        # set_trace()
        visualize =0& global_visualize
        # visualize =1
        if visualize:
            ...
            ax2[0,1].imshow(labp1.reshape(sz1,sz2).astype(np.uint8))
...
def track_vos(model, video, hp=None, mask_enable=False, refine_enable=False, mot_enable=False, device='cpu',semip=1):
    ...
    if semip:
        ...
        for f, image_file in enumerate(image_files):
            im = cv2.imread(image_file)
            save_name = video['name']+'_'+str(f)
            labp = semip_predict(model, im, rf_model, lr_model, save_name)
            labp_list.append(labp)
            # if f>3:break
        toc_semip =cv2.getTickCount()
        toc_semip=(toc_semip - tic_semip)/cv2.getTickFrequency()
    print("number of images:"+str(len(image_files))+ " number of objects:"+str(len(object_ids)))
    for obj_id, o_id in enumerate(object_ids):
        ...
        for f, image_file in enumerate(image_files):
            ...
            if f == start_frame:  # init
                ...
                target_pos = np.array([cx, cy])
                target_sz = np.array([w, h])
                state = siamese_init(im, target_pos, target_sz, model, hp, device=device,  semip =semip)  # init tracker
                state['frame_id'] =0
                if semip:
                ...
                if semip:
                    labp = labp_list[f][:,:,obj_id+1]
                else:
                    labp=0
                state = siamese_track(state, im, mask_enable, refine_enable, device=device,visualize_ =visualize_,save_name = save_name, labp=labp)  # track
                # mask = state['mask']
                mask = state['mask_semip']
            toc += cv2.getTickCount() - tic
    ...
    if semip:
        ...
        print("semip time:"+ str(toc_semip))# if semip:
        # toc = toc*2
        # toc_semip = state['semip_time']
    toc /= cv2.getTickFrequency()
    ...
    # guess = mask
    pred_masks= masks_semip - optim_seg_bias
        # - optim_seg_thr
    ...
    if args.save_mask:
        ...
        # if diffuse:
        video_path = join('test_diffuse5', args.dataset, 'SiamMask', video['name'])
        if not isdir(video_path): makedirs(video_path)
        ...
        # else:
        #     postfix2='_difmask.png'
        #     postfix= '_difmask_show.png'
        postfix2='_difmask.png'
        postfix= '_difmask_show.png'
        for i in range(pred_mask_final.shape[0]):
            ...
            # set_trace()
            middlefilename= image_files[i].split('\\')[-1].split('.')[0]
            filename =join(video_path, middlefilename+ postfix)
            filename2 =join(video_path, middlefilename + postfix2)
            # set_trace()
            cv2.imwrite(filename, output)
            cv2.imwrite(filename2, output2)
            # cv2.imshow("mask_diffuse", output)
            # cv2.waitKey(1)
    
    save_mask_base = False
    if save_mask_base:
        seg_thr =  state['p'].seg_thr
        video_path = join('test_diffuse4', args.dataset, 'SiamMask', video['name'])
        if not isdir(video_path): makedirs(video_path)
...
def main():
    ...
    global_tic = cv2.getTickCount()
    video_list= list(enumerate(dataset.keys(), start=1))
    for v_id, video in video_list:
        print("video id:"+str(v_id)+" out of "+str(len(video_list)))
        time = (cv2.getTickCount()- global_tic)/cv2.getTickFrequency()/3600
        print("used "+str(time)+" hours")
