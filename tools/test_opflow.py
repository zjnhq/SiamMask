...
def semip_predict(net, im, rf_model, lr_model, save_name="save_name", multiclass=1):
    ...
    return labp
def siamese_track(state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False, visualize_=0,save_name='exp', labp='0'):
    p = state['p']
    net = state['net']
    ...
    # optical flow
    prvs = state['prvs'] 
    init_frame_gray = state['init_frame_gray']
    prvs= cv2.cvtColor(prvs,cv2.COLOR_BGR2GRAY)
    im_gray = cv2.cvtColor(im,cv2.COLOR_BGR2GRAY)
    flow = cv2.calcOpticalFlowFarneback(prvs, im_gray, None, 0.5, 3, 15, 3, 5, 1.2, 0)# flow from n-th to (n+1)-th frame

    flow2init_frame = cv2.calcOpticalFlowFarneback(im_gray, init_frame_gray, None, 0.5, 3, 15, 3, 5, 1.2, 0)
    # (flow)
    x= im_gray
    # x: [B, C, H, W] (im2)
    # flo: [B, 2, H, W] flow
    ...
    # if x.is_cuda:
    grid = grid.to(device)
    flow = Variable(torch.from_numpy(flow)).to(device).unsqueeze(0).permute(0,3,1,2)
    set_trace()
    ...
    labp=torch.ones(x.shape).unsqueeze(0).unsqueeze(0)
    mask = torch.autograd.Variable(labp).cuda()
    mask = F.grid_sample(mask, vgrid) #"""

    net_consistency.inference(m)
    ...
    for obj_id, o_id in enumerate(object_ids):
        ...
        for f, image_file in enumerate(image_files):
            ...
            if f == start_frame:  # init
                ...
                if optflow_:
                    state['prvs'] = im
                    state['init_frame_gray']= cv2.cvtColor(im,cv2.COLOR_BGR2GRAY)
