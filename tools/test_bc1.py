...
def get_negative_pixels(im, pos, model_sz, original_sz, avg_chans):
    if isinstance(pos, float):
        pos = [pos, pos]
    sz = original_sz
    im_sz = im.shape
    ...
    im_patch_copy[int(context_ymin):int(context_ymax + 1), int(context_xmin):int(context_xmax + 1)] = 0

    return im_patch_copy
...
def siamese_init(im, target_pos, target_sz, model, hp=None, device='cpu', take_feat =False):
    state = dict()
    ...
    #     ax[0].imshow(neg_img)
    #     ax[ 0].set_title("negative pixels")
    #     ax[ 1].imshow(template_img)
    #     ax[ 1].set_title('positive pixels')
    #     plt.show()
    # seg_labels = crf_seg(template_img,neg_img)
    # seg_labels = seg_labels.reshape(-1)
    # template_img = template_img.reshape(-1,3)
    # # template_img
    # rf_model = train_rf(template_img,seg_labels)
...
def siamese_track(state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    ...
    template_img=get_subwindow_tracking(im, target_pos, p.instance_size, round(s_x), avg_chans,out_mode='img')
    # crf_seg(template_img)
    # set_trace()
    # if semip:
    #     (sz1,sz2) = template_img.shape
    #     pixels = template_img.reshape(-1,3)
    #     seg_labels = state['rf_model'].predict(pixels).reshape(sz1,sz2)
        # seg_labels = 
    ...
    visualize= 1
    def deeprf_predict(llfeat, rf_model):
        # set_trace()
        nchannel = llfeat.shape[1]
        # (sz1,sz2) = mask.shape
        sz1,sz2 = llfeat.shape[2],llfeat.shape[3]

        llfeat_resz = interpolate(llfeat, size=[sz1,sz2],  mode='nearest', align_corners=None).detach()
    ...
    if semip and mask_enable:
        ...
        # llfeat = net.search
        # llfeat = net.features(im_to_torch(im).unsqueeze(0))#.squeeze(0)
        llfeat = net.features2(im_to_torch(im).unsqueeze(0))#.squeeze(0)

        # set_trace()
        # print("llfeat shape"+str(llfeat.shape))
        nchannel = llfeat.shape[1]
        # (sz1,sz2) = mask.shape
        # sz1,sz2 = llfeat.shape[2],llfeat.shape[3]
        # print("im.shape")
        ...
        # sz1f,sz2f = sz1*8,sz2*8

        # feat1 = llfeat.detach()
        # mask_r = cv2.resize(mask, dsize=(sz1f, sz2f), interpolation=cv2.INTER_CUBIC)
        # llfeat_resz = interpolate(llfeat, size=[sz1f,sz2f],  mode='nearest', align_corners=None).detach()
        ...
        use_svm =0
        if use_svm:
            labp2 =lr_model.predict_proba(pixels)[:,0]
            ...
            # labp = np.maximum(sv_model.decision_function(labp_concat),-0.1)+0.1
        labp = labp.reshape(sz1,sz2)
        # labp_concat = np.concatenate((labp1,labp2),axis=1)
        # labp_concat = np.concatenate((labp1[:,np.newaxis],labp2[:,np.newaxis]),axis=1)
        # # labp = sv_model.predict_proba(labp_concat,lab)[:,0]
        # labp = np.maximum(sv_model.decision_function(labp_concat),-0.1)+0.1
        # # labp = labp1 + labp2
        # labp = labp.reshape(sz1,sz2)
        # set_trace()
        # maskp = deeprf_predict(llfeat,rf_model)
        ...
        if visualize:
            ...
            ax[0,1].set_title('labp1')
            ax[1,0].imshow(labp2.reshape(sz1,sz2))
            ax[1,0].set_title('labp2')
            ax[1,1].imshow(labp)
            ax[1,1].set_title('labp')
            plt.show()
    ...
    if mask_enable:
        ...
        if semip:
            ...
            target_ind = np.where(mask_in_img > (p.seg_thr-0.05))
            mask_in_img[target_ind] = mask_in_img[target_ind] + labp[target_ind]*0.1-0.05
        # else:
    ...
    if semip: state['semip_tic'] = state['semip_tic'] + toc_semip
    return state
...
def track_vos(model, video, hp=None, mask_enable=False, refine_enable=False, mot_enable=False, device='cpu',semip=1):
    ...
    for obj_id, o_id in enumerate(object_ids):
        ...
        for f, image_file in enumerate(image_files):
            ...
            if f == start_frame:  # init
                ...
                if semip:
                    # set_trace()
                    # shallow_model = model.features.features
                    # lowlevel_feat = shallow_model.forward_lowlevel(im_to_torch(im))
                    ...
                    rf_model = RandomForestClassifier(max_depth=6, n_estimators=10,  n_jobs=-1)
                    lr_model = LogisticRegression(C=1.0, random_state=0, solver='lbfgs', max_iter=30, n_jobs=-1)
                    ...
                    def deeprf_train(llfeat,mask, rf_model,lr_model):
                        ...
                        if visualize:
                            ...
                            ax[0,0].set_title("pixels")
                            # ax[0,1].imshow(labp)
                            # ax[0,1].set_title('lab pre')
                            ax[0,1].imshow((labp1.reshape(sz1,sz2)*100).astype(np.uint8))
                            ax[0,1].set_title('labp1')
    ...
    print("overall time:"+ str(toc/10000000000.0))

    if semip:
        toc_semip = state['semip_tic']
        print("semip time:"+ str(toc_semip/10000000000.0))# if semip:
        toc = toc*2
        # toc_semip = state['semip_tic']
    toc /= cv2.getTickFrequency()
    ...
    if args.save_mask:
        ...
        if semip:
            for i in range(pred_mask_final.shape[0]):
                output = ((0.7 * cv2.imread(image_files[i])) + (0.3 * mask[i,:,:,:])).astype("uint8")
                filename =join(video_path, image_files[i].split('/')[-1].split('.')[0] + '_jq.png')
                cv2.imwrite(filename, output)
                # cv2.imwrite(filename, pred_mask_final[i].astype(np.uint8))
        else:
            for i in range(pred_mask_final.shape[0]):
                output = ((0.7 * cv2.imread(image_files[i])) + (0.3 * mask[i,:,:,:])).astype("uint8")
                filename = join(video_path, image_files[i].split('/')[-1].split('.')[0] + '.png')
                cv2.imwrite(filename, output)
                # cv2.imwrite(filename, pred_mask_final[i].astype(np.uint8))
